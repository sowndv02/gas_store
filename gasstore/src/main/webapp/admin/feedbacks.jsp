<%-- 
    Document   : feedbacks
    Created on : May 20, 2023, 6:38:15 PM
    Author     : ADMIN
--%>
                                ...
                                <div class="comment-widgets scrollable">
                                    ...
                                    <c:forEach items="${requestScope.listAllFeedbacks}" var="f">
                                        <div class="d-flex flex-row comment-row mt-0">
                                            <c:choose>
                                                <c:when test="${not empty f.customer}">
                                                    <div class="p-2">
                                                        <img src="data:image/jpg;base64,${f.customer.base64Image}"
                                            ...
                                            <div class="comment-text w-100">
                                                <c:choose>
                                                    <c:when test="${not empty f.customer}"><h6 class="font-medium"><a href="${pageContext.request.contextPath}/admin/profile?type=customer&id=${f.customer.customerID}">${f.customer.firstName} ${f.customer.lastName}</a> (<span>${f.sendDate}</span>)</c:when>
                                                        <c:when test="${not empty f.supplier}"><h6 class="font-medium"><a href="${pageContext.request.contextPath}/admin/profile?type=sup&id=${f.supplier.supplierId}">${f.supplier.companyName}</a> (<span>${f.sendDate}</span>)</c:when>
                                                            <c:when test="${not empty f.shipment}"><h6 class="font-medium"><a href="${pageContext.request.contextPath}/admin/profile?type=ship&id=${f.shipment.shipmentId}">${f.shipment.companyName}</a> (<span>${f.sendDate}</span>)</c:when>
                                                                <c:otherwise>${f.name}</c:otherwise>
                                                            </c:choose>
                                                            <c:choose>
                                                                <c:when test="${f.status}">/(<span>${f.repDate}</span>)<i class="fas fa-check-circle"></i></c:when>
                                                        ...
                                                        <div class="comment-footer">
                                                            ...
                                                            <c:if test="${!f.status}">
                                                                <button type="button" class="btn btn-success btn-sm
                                                                        text-white">
                                                                    <a href="${pageContext.request.contextPath}/admin/reply?id=${f.feedbackId}" style="color: white">Trả lời</a>
                                                                </button>
                                                            </c:if>

                                                            <button type="button" class="btn btn-info btn-sm
                                                                    text-white">
                                                                <a href="${pageContext.request.contextPath}/admin/reply?id=${f.feedbackId}" style="color: white">Chi tiết</a>
                                                            </button>
                                                        </div>
                                                        </div>
                                                        </div>
