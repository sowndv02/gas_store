...
public class AddCategoryAdminServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddCategoryAdminServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddCategoryAdminServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (categoryName.isEmpty() || code.isEmpty()) {
            request.setAttribute("message", "Không được bỏ trống code và tên!!");
        } else {
            Category cateName = dao.getCategoryByCol("name", categoryName);
            Category cateCode = dao.getCategoryByCol("code", code);
            if (cateName != null || cateCode != null) {
                request.setAttribute("message", "Code hoặc Name đã tồn tại!!");
            } else {
                Category c = new Category(code, categoryName, keyword);
                c.setDescription(des);
                Administrator admin = new Administrator();
                admin.setAdministratorId(1);
                c.setAdmin(admin);
                int number = dao.AddNewCategory(c);
                if (number > 0) {
        ...
        Vector<Category> listAllCate = dao.getAllCategories();
        request.setAttribute("listAllCate", listAllCate);
        request.getRequestDispatcher("categories.jsp").forward(request, response);
    }
